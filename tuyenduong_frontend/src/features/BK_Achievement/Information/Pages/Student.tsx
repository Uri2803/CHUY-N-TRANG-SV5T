...
let validationSchema = yup.object().shape({
  ...
  emailPersonal: yup
    ...
    .max(50, "Không được vượt quá 50 ký tự"),
  department: yup.string().required("Bắt buộc nhập").nullable(true),
  // youthBranch: yup.string().required("Bắt buộc nhập").nullable(true),
  phone: yup
    .string()
...
const Student: React.FC<Props> = (props) => {
  ...
  }, [props.token, user.id, user.role, history]);
