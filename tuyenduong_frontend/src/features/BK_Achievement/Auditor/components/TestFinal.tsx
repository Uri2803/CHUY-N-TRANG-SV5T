...
const SubmissionUsersTestFinal = (props: any) => {
  ...
  useEffect(() => {
    async function getUsers() {
      try {
        const { data } = await AuditorApi.getUsersFinal(
          achievementId,
          auditorId,
          filters
        );
  ...
  useEffect(() => {
    async function getSubmissionExamersFinal() {
      try {
        const res = await AuditorApi.getSubmissionExamersFinal(
          achievementId,
          userId,
          ...
          <CCard style={well}>
            <CCardBody className="d-flex">
              ...
              <CCol sm="4">
                <CSelect
                  onChange={(e) => {
                    // alert(typeof e.currentTarget.value)
...
export default memo(SubmissionUsersTestFinal);
